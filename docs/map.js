...
function openArticle(el) {
  ...
  if (!m) return true; // fall back to normal link if modal missing
  ...
  return false; // cancel new-tab navigation (ctrl/middle-click still opens a tab)
}
...
function computeWeeklyStats(articles) {
  ...
  bar.innerHTML = `<span class="week-pill">7 days</span>${recent.length} articles · ${places.size} places${topCat ? ` · <span class="week-top">${escapeHtml(CATEGORY_LABELS[topCat[0]])}</span>` : ''}`;
}
